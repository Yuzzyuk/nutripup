...
function MainComponent() {
  ...
  const calculateNutritionScore = (meals) => {
    ...
    return {
      protein: Math.min(100, (totalNutrition.protein / 50) * 100),
      fats: Math.min(100, (totalNutrition.fat / 15) * 100),
      minerals: Math.min(100, Math.random() * 80 + 20),
      vitamins: Math.min(100, Math.random() * 80 + 20),
      energy: Math.min(100, (totalNutrition.calories / 800) * 100),
      fiber: Math.min(100, Math.random() * 80 + 20),
      calcium: Math.min(100, Math.random() * 80 + 20),
      phosphorus: Math.min(100, Math.random() * 80 + 20),
    };
  };
  ...
  const getMealFeedback = (meal) => {
    ...
    if (meal.protein < 20) {
      feedback.push("Add more protein sources like chicken or fish");
    }
    if (meal.fat < 5) {
      feedback.push(
        "Include healthy fats like salmon or a small amount of olive oil"
      );
    }
    if (
      meal.carbs < 10 &&
      ...
      feedback.push("Consider adding some vegetables for fiber and nutrients");
    }
    if (meal.calories < 100) {
      feedback.push("This is a light meal - consider increasing portion size");
    }

    if (feedback.length === 0) {
      feedback.push("Great choice! This is a well-balanced ingredient");
    }
  ...
  const getSuggestions = () => {
    ...
    if (nutritionScore.protein < 70)
      suggestions.push("Add lean protein like chicken or fish");
    if (nutritionScore.fats < 60)
      suggestions.push("Include healthy fats like salmon or fish oil");
    if (nutritionScore.fiber < 50)
      suggestions.push("Add fiber-rich vegetables like sweet potato");
    if (suggestions.length === 0)
      suggestions.push("Great job! Your pup's nutrition looks balanced today");
    return suggestions;
  };
        ...
        <div className="container mx-auto px-4 py-8 max-w-md">
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 mb-6">
            ...
            <div className="flex justify-center mb-6 relative">
              ...
              {/* External labels positioned absolutely */}
              {radarPoints.map((point) => {
                const angleRad = (point.angle * Math.PI) / 180;
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 mb-6">
            ...
            <div className="space-y-3">
              {getSuggestions().map((suggestion, index) => (
                <div key={index} className="p-3 bg-[#f7f3f0] rounded-xl">
                  <p className="text-[#8b7355]">{suggestion}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {healthFocusOptions.map((option) => (
                  <button
                    key={option.id}
                    onClick={() => {
                      const newFocus = dogProfile.healthFocus.includes(
                        option.id
                      )
                        ? dogProfile.healthFocus.filter((f) => f !== option.id)
                        : [...dogProfile.healthFocus, option.id];
                      setDogProfile({ ...dogProfile, healthFocus: newFocus });
                    }}
                    className={`p-3 rounded-2xl text-sm font-medium transition-all ${
                      dogProfile.healthFocus.includes(option.id)
                        ? "bg-[#9db5a1] text-white"
                        : "bg-[#f7f3f0] text-[#8b7355] hover:bg-[#e8ddd4]"
                    }`}
                  >
        ...
        <div className="container mx-auto px-4 py-6 max-w-md">
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 mb-6">
            ...
            <div className="relative mb-4">
              ...
              />
              <i className="fas fa-search absolute right-4 top-1/2 transform -translate-y-1/2 text-[#a0916b]"></i>
            </div>
              ...
              <div className="space-y-2 mb-4 max-h-48 overflow-y-auto">
                {filteredIngredients.map((ingredient) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-[#a0916b]">
                      {ingredient.protein}g protein • {ingredient.calories}{" "}
                      cal/100g
                    </div>
                  </button>
                ))}
                  ...
                  <div key={meal.id} className="p-3 bg-[#f7f3f0] rounded-xl">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        onClick={() =>
                          setDailyMeals(
                            dailyMeals.filter((m) => m.id !== meal.id)
                          )
                        }
                        className="text-[#a0916b] hover:text-red-500 transition-colors"
                      >
        ...
        <div className="container mx-auto px-4 py-6 max-w-md">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-[#a0916b]">
                {dogProfile.name}'s daily nutrition
              </p>
            </div>
          ...
          {mealHistory.length === 0 ? (
            <div className="bg-white rounded-3xl shadow-lg p-8 text-center">
              ...
              <p className="text-[#a0916b] mb-6">
                Begin tracking {dogProfile.name}'s nutrition to see progress
                over time
              </p>
              <button
                onClick={() => setCurrentStep("meals")}
                  ...
                  <div className="space-y-2">
                    {day.meals.slice(0, 3).map((meal, mealIndex) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-[#a0916b]">
                          {meal.portion}g
                        </span>
                      </div>
                    ))}
